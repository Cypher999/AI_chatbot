...
export default ({id})=> {
  ...
  const readData=async()=>{
    ...
    let req=await axios.get(process.env.NEXT_PUBLIC_API_URL+'user/knowledge/'+id,{
      headers:{
  ...
  const readBot=async()=>{
    setLoading(true);
    let token=Cookies.get('auth-token')
    let req=await axios.get(process.env.NEXT_PUBLIC_API_URL+'user/agent/'+id,{
      headers:{
        authorization:"bearer "+token
      }
    })
  ...
  const readOne=async(knowledgeId)=>{
    let token=Cookies.get('auth-token')
    let req=await axios.get(process.env.NEXT_PUBLIC_API_URL+'user/knowledge/'+id+'/'+knowledgeId,{
      headers:{
        authorization:"bearer "+token
      }
    })
  ...
  const del=async(knowledgeId)=>{
    let token=Cookies.get('auth-token')
    let req=await axios.delete(process.env.NEXT_PUBLIC_API_URL+'user/knowledge/'+id+'/'+knowledgeId,{
      headers:{
        authorization:"bearer "+token
      }
    })
