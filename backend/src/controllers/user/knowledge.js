...
const index=async(req,res)=>{
    ...
    const data=await knowledgeUtils.getMany({agentId,agent:{userId:req.user_id}});
...
const del=async(req,res)=>{
    ...
    if (oldData==null) return res.status(404).json({status:'error',
        message:"knowledge not found"})
    const result=await knowledgeUtils.del({id,agent:{userId:req.user_id}})
